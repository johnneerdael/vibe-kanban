...
- **From existing git repository**: Browse the container's `/repos` directory and select from a list of git repositories.
- **Create blank project**: Generate a new git repository from scratch inside the `/repos` directory.

Each project represents a git repository. Since Vibe Kanban runs in Docker, all repositories must be located within the volume mapped to `/repos`.
...
<Note>
Ensure these files exist in the project's main directory within the `/repos` volume.
</Note>

...
<Step title="Configure Development Server Script">
  Navigate to your project settings and configure the development server script. Since Vibe Kanban runs in Docker, this script will execute inside the container environment.
  ...
  **Common examples:**
  ...
  <Tip>
  Ensure your development server prints the URL (e.g., `http://localhost:3000`) to stdout/stderr for automatic detection. Vibe Kanban will automatically proxy this internal container port to the web UI.
  </Tip>
</Step>
